...
public class Ordenamiento {
        ...
        for(int i = (X.length-2)/2; i>=0; i--){
            ...
            while(!hecho){//2*i+1, 2*i+2
                if(2*current+1 > X.length-1){
                    ...
                    //el nodo actual tiene al menos un hijo, obtenga el indice del hijo mas grande
                    maxChildIndex = HeapSortMaxChild(X, current, X.length-1);
                    if(X[current]<X[maxChildIndex]){
                        intercambio(X, current, maxChildIndex);
                        current = maxChildIndex;
                    }
                    else{//la restriccion de la relacion de valor se cumple, terminar
                        hecho = true;
                    }
                }
            }        
    ...
    private Integer[] HeapSortExtract(Integer[] X){
        ...
        for(int i=X.length; i>=0; i--){
            Y[i]=X[0];// colocando el nodo raiz en la poscion actual del nuevo heap
            X[0]=X[i];
            current = 0;
            hecho = false;
            while(!hecho){
